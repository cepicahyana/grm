...
class Dlantamal extends CI_Controller {
	...
	function data_tables()
	{
		...
		foreach ($list as $dataDB) {
			...
			$id_admin=isset($dataDB->id_admin)?($dataDB->id_admin):'';
			...
			$tombol='
					<button type="button" onclick="wilayahkerja(`'.$id.'`)" class="btn bg-success btn-sm">WILAYAH KERJA</button>
...
*/	
	...
	{
		$data["data"]=$this->mdl->edit_data();
		echo $this->load->view("formEditwilayahkerja",$data);
	}
	function update_wilayah_kerja()
	{
		$data=$this->mdl->update_wilayah_kerja();
	 	echo json_encode($data);
	}
