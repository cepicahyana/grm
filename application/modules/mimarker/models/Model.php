...
class Model extends ci_Model
{
	...
	function insert_data()
	{
		...
		$img=$this->m_reff->upload_file("icon","theme/images/marker/","filemarker","PNG","10000",null);
		if($img['validasi']==true){
			$this->db->set("icon",$img['name']);
		}
		//$this->db->set("level","6");
	...
	function update_data()
	{
		...
		$icon_b=$this->input->post("icon_b");

		//$tanggal_lahir=$this->input->post("tanggal_lahir");
		//$tgl_lahir=$this->tanggal->eng_($tanggal_lahir,0);
		//cek dulu
		$this->db->where("nama!=",$nama_b);
		$this->db->where('nama',$nama);
		$cek=$this->db->get($this->tbl)->num_rows();
		if($cek)
		{
		...
		$img=$this->m_reff->upload_file("icon","theme/images/marker/","filemarker","PNG","5000",$icon_b);
		if($img['validasi']==true){
			$this->db->set("icon",$img['name']);
		}
		//$this->db->set("level","6");
	...
	function delete_item()
	{
		...
		$image=isset($del->icon)?($del->icon):'';
		
		if($image!=null){
			$structure = './theme/images/marker/'.$image.'';
						...
						$kd_kelas=isset($kelasDB->kode)?($kelasDB->kode):'';
						 ...
						 if($cek_ktp)
						 {
							...
							//	 $kode_gardu=sprintf("%03s", $kode_gardu);
								...
								$cek1=$this->db->get_where($this->tbl,array("nama"=>$nama))->num_rows();
								if($cek1){	
									$dataray=array(
										"nama"=>$nama,
										"descdata"=>$descdata,
	...
	{
		$this->db->where("nama",$dataray['nama']);
