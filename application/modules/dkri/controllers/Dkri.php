...
class Dkri extends CI_Controller {
	...
	function data_tables()
	{
		...
		foreach ($list as $dataDB) {
			...
			$dlevel=$this->m_konfig->dataLevel($level);
  			$levelname=isset($dlevel->levelname)?($dlevel->levelname):'';
		
			
			$tombol='
					<button type="button" onclick="edit(`'.$id.'`)" class="btn bg-info btn-sm">EDIT</button>
					<button type="button" onclick="del(`'.$id.'`,`'.$profilename.'`)" class="btn bg-danger btn-sm">DELETE</button>
			';
			$row = array();
			$row[] = "";
			$row[] = $id;	
			...
			$row[] = "<span class='size' >Username : ".$username."<br>Profilename : ".$profilename."<br>Status : ".$sts."</span>";
			$row[] = $tombol ;
