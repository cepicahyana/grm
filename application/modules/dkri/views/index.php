				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Data KRI</h4>
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<div class="row">
										<div class="col-6">
										<h4 class="card-title">Kelola data KRI</h4>
										</div>
										<div class="col-6">
								...
								<div class="card-body">
									<!--div class="row">
										...
										<div class="form-group">
											...
											<select id="f1" class="form-control select2" onchange="reload_table()" style="width:100%">
												<option value="">=== All ===</option>
												<.?php 
												$db=$this->db->order_by("code_level","asc");
												$db=$this->db->where("code_level>",'2');
												$db=$this->db->get("main_level")->result();
												foreach($db as $val){
												echo "<option value='".$val->code_level."'>".$val->levelname."</option>";
												}
												?>
											</select>
										</div>
									...
									<div id="area_lod">
										...
										<table id='table' class="tabel black table-bordered  table-hover dataTable" style="font-size:12px;width:100%">
											<thead  class='sadow bg-teal'>
												...
												<th class='thead' >NAMA KRI</th>
												<th class='thead' >AKUN </th>
												<th class='thead' style="min-width:130px">AKSI</th>
											</thead>
										</table>
										</div>
									</div>
...
<script type="text/javascript">	
	var  dataTable = $('#table').DataTable({ 
		...
		"ajax": {
			"url": "<?php echo site_url('dkri/data_tables');?>",
			"type": "POST",
			"data": function ( data ) {
				data.f1 = $('#f1').val();
			 },
...
function input()
{ 
	$("#title_mdl_input").html("INPUT DATA KRI");
	$("#mdl_formSubmit_input").modal({backdrop: 'static', keyboard: false});
	$("#add_page").html('<center>Please wait..</center>');
	$("#formSubmit_input").attr("url","<?php echo base_url("dkri/insert_data");?>");
	$.post("<?php echo site_url("dkri/input_data"); ?>",function(data){
		$("#add_page").html(data);
		$("#formSubmit_input")[0].reset();
		$("#inputpreview_img").attr("src", '<?php echo base_url()?>theme/images/user/img_not.png');
		$("#inputpreview_img2").attr("src", '<?php echo base_url()?>theme/images/no-image.png');
	});	
}
function edit(id)
{	
	$("#title_mdl_edit").html("EDIT DATA KRI");
	$("#mdl_formSubmit_edit").modal({backdrop: 'static', keyboard: false});
	$("#edit_page").html('<center>Please wait..</center>');
	$("#formSubmit_edit").attr("url","<?php echo base_url("dkri/update_data");?>");
	$.post("<?php echo site_url("dkri/edit_data"); ?>",{id:id},function(data){
		$("#edit_page").html(data);
	});
}
function priview(id)
{	
	$("#title_mdl_view").html("PRIVIEW DATA KRI");    
    $("#mdl_formSubmit_view").modal({backdrop: 'static', keyboard: false});	
	$("#view_page").html('<center>Please wait..</center>');
	$.post("<?php echo site_url("dkri/view_data"); ?>",{id:id},function(data){
		$("#view_page").html(data);
	});
}
function del(id,name){
   alertify.confirm("Delete","<center>Delete data <b>Name : "+name+"</b>?</center>",function(){
		$.ajax({
			 url:'<?php echo site_url("dkri/delete_item"); ?>',
			 data: 'id='+id,
			 method:"POST",
			 dataType:"JSON",
...
{ 	//alert('');
	...
	$("#title_mdl_down").html("IMPORT DATA");
	//$("#isi").html(data);
	$("#mdl_formSubmitDown").modal('show');
	$("#formSubmitDown").attr("url","<?php echo base_url()?>data_member/import_data");
}
