...
<script type="text/javascript">	
	var  dataTable = $('#table').DataTable({ 
		...
		"ajax": {
			"url": "<?php echo site_url('apengumuman/data_tables');?>",
			"type": "POST",
			"data": function ( data ) {
				//data.f1 = $('#f1').val();
			 },
...
function input()
{ 
	$("#title_mdl_input").html("INPUT DATA PENGUMUMAN");
	$("#mdl_formSubmit_input").modal({backdrop: 'static', keyboard: false});
	$("#add_page").html('<center>Please wait..</center>');
	$("#formSubmit_input").attr("url","<?php echo base_url("apengumuman/insert_data");?>");
	$.post("<?php echo site_url("apengumuman/input_data"); ?>",function(data){
		$("#add_page").html(data);
...
function edit(id)
{	
	$("#title_mdl_edit").html("EDIT DATA PENGUMUMAN");
	$("#mdl_formSubmit_edit").modal({backdrop: 'static', keyboard: false});
	$("#edit_page").html('<center>Please wait..</center>');
	$("#formSubmit_edit").attr("url","<?php echo base_url("apengumuman/update_data");?>");
	$.post("<?php echo site_url("apengumuman/edit_data"); ?>",{id:id},function(data){
		$("#edit_page").html(data);
	});
}
function priview(id)
...
function del(id,name){
   alertify.confirm("Delete","<center>Delete data <b>Name : "+name+"</b>?</center>",function(){
		$.ajax({
			url:'<?php echo site_url("apengumuman/delete_item"); ?>',
			data: 'id='+id,
			method:"POST",
		 	dataType:"JSON",
			beforeSend: function() {
...
{ 	//alert('');
	...
	$("#title_mdl_down").html("IMPORT DATA PENGUMUMAN");
	//$("#isi").html(data);
	$("#mdl_formSubmitDown").modal('show');
	$("#formSubmitDown").attr("url","<?php echo base_url()?>apengumuman/import_data");
}
	...
	$('.delsel').on('click', function(e){ 
		...
		if(sData!=''){
			alertify.confirm("Delete selected","<center>Delete "+oData.length+" data ?</center>",function(){
					$.ajax({
						url:'<?php echo site_url("apengumuman/delete_sel"); ?>',
						data: 'ct='+sData,
						method:"POST",
						dataType:"JSON",
						beforeSend: function() {
