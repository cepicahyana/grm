				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Data BRIGIF</h4>
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<div class="row">
										<div class="col-6">
										<h4 class="card-title">Kelola data BRIGIF</h4>
										</div>
										<div class="col-6">
...
<script type="text/javascript">	
	var  dataTable = $('#table').DataTable({ 
		...
		"ajax": {
			"url": "<?php echo site_url('dbrigif/data_tables');?>",
			"type": "POST",
			"data": function ( data ) {
				//data.f1 = $('#f1').val();
			 },
...
function input()
{ 
	$("#title_mdl_input").html("INPUT DATA BRIGIF");
	$("#mdl_formSubmit_input").modal({backdrop: 'static', keyboard: false});
	$("#add_page").html('<center>Please wait..</center>');
	$("#formSubmit_input").attr("url","<?php echo base_url("dbrigif/insert_data");?>");
	$.post("<?php echo site_url("dbrigif/input_data"); ?>",function(data){
		$("#add_page").html(data);
...
function edit(id)
{	
	$("#title_mdl_edit").html("EDIT DATA BRIGIF");
	$("#mdl_formSubmit_edit").modal({backdrop: 'static', keyboard: false});
	$("#edit_page").html('<center>Please wait..</center>');
	$("#formSubmit_edit").attr("url","<?php echo base_url("dbrigif/update_data");?>");
	$.post("<?php echo site_url("dbrigif/edit_data"); ?>",{id:id},function(data){
		$("#edit_page").html(data);
	});
}
function priview(id)
{	
	$("#title_mdl_view").html("PRIVIEW DATA BRIGIF");    
    $("#mdl_formSubmit_view").modal({backdrop: 'static', keyboard: false});	
	$("#view_page").html('<center>Please wait..</center>');
	$.post("<?php echo site_url("dbrigif/view_data"); ?>",{id:id},function(data){
		$("#view_page").html(data);
	});
}
function del(id,name){
   alertify.confirm("Delete","<center>Delete data <b>Name : "+name+"</b>?</center>",function(){
		$.ajax({
			url:'<?php echo site_url("dbrigif/delete_item"); ?>',
			data: 'id='+id,
			method:"POST",
		 	dataType:"JSON",
			beforeSend: function() {
...
{ 	//alert('');
	...
	$("#title_mdl_down").html("IMPORT DATA BRIGIF");
	//$("#isi").html(data);
	$("#mdl_formSubmitDown").modal('show');
	$("#formSubmitDown").attr("url","<?php echo base_url()?>dbrigif/import_data");
}
	...
	$('.delsel').on('click', function(e){ 
		...
		if(sData!=''){
			alertify.confirm("Delete selected","<center>Delete "+oData.length+" data ?</center>",function(){
					$.ajax({
						url:'<?php echo site_url("dbrigif/delete_sel"); ?>',
						data: 'ct='+sData,
						method:"POST",
						dataType:"JSON",
						beforeSend: function() {
